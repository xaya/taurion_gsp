...
class RoConfigTest (PXTest):
  ...
  def run (self):
    ...
    self.mainLogger.info ("Changing a building shape...")
    # Building shapes are read from the config as well.  Since merging
    # replaces a map entry as a whole, the update carries the full building
    # data with just the shape changed.
    shape = lambda: self.getRpc ("getbuildingshape", type="checkmark",
                                 centre={"x": 0, "y": 0}, rot=0)
    baseShape = shape ()
    assert len (baseShape) > 1
    upd = config_pb2.ConfigData ()
    upd.building_types["checkmark"].CopyFrom (
        self.roConfig ().building_types["checkmark"])
    del upd.building_types["checkmark"].shape_tiles[1:]
    self.sendConfigUpdate (upd)
    self.assertEqual (shape (), [{"x": 0, "y": 0}])
    ...
    self.assertEqual (shape (), baseShape)
    self.moveWithPayment ("domob", {"nc": [{}]}, baseCost)
    self.generate (1)
    assert "domob" in self.getCharacters ()
