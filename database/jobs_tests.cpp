...
protected:
  ...
  /**
   * Creates an OPEN ad-slot job with a deadline and linked building, and
   * returns the handle so the caller can fill in more.  A generic sample of
   * a deadlined, entity-linked job for the table-layer tests.
   */
  JobsTable::Handle
  CreateAd (const Faction f, const std::string& poster,
            const Amount reward, const Amount collateral,
            const int64_t deadline, const Database::IdT building)
  {
    auto j = tbl.CreateNew (Job::Type::AD, f, poster, reward, collateral);
    j->SetDeadline (deadline);
    j->SetLinkedId (building);
    return j;
  }
...
TEST_F (JobsTableTests, RoundTrip)
{
  Database::IdT id;
  {
    auto j = CreateAd (Faction::RED, "poster", 2000, 8000, 1234, 42);
    j->MutableProto ().mutable_ad ()->set_slot (5);
    id = j->GetId ();
  }

  auto j = tbl.GetById (id);
  ASSERT_NE (j, nullptr);
  EXPECT_EQ (j->GetType (), Job::Type::AD);
  ...
  EXPECT_EQ (j->GetProto ().ad ().slot (), 5);
}
...
TEST_F (JobsTableTests, WorkerNullThenSet)
{
  Database::IdT id;
  {
    auto j = CreateAd (Faction::RED, "poster", 100, 50, 10, 1);
    id = j->GetId ();
  }
...
{
  db.SetNextId (101);
  CreateAd (Faction::RED, "poster", 1, 0, 50, 1).reset ();
  CreateAd (Faction::RED, "poster", 1, 0, 100, 1).reset ();
  CreateAd (Faction::RED, "poster", 1, 0, 150, 1).reset ();
  /* A standing job with no deadline -- must never appear in the sweep.  */
  {
    auto s = tbl.CreateNew (Job::Type::WANTED, Faction::INVALID, "poster", 1, 0);
...
{
  db.SetNextId (201);
  CreateAd (Faction::RED, "poster", 1, 0, 10, 42).reset ();
  CreateAd (Faction::RED, "poster", 1, 0, 10, 43).reset ();
  CreateAd (Faction::RED, "poster", 1, 0, 10, 42).reset ();
...
TEST_F (JobsTableTests, QueryForLinkedName)
{
  ...
  /* Two bounties on the same name (stacked pools), one on another, and a
     ad row whose NULL linked_name must never match.  */
  ...
  tbl.CreateNew (Job::Type::WANTED, Faction::INVALID, "poster", 100, 0)
      ->SetLinkedName ("badguy");
  CreateAd (Faction::RED, "poster", 1, 0, 10, 1).reset ();
...
{
  /* A NULL linked_name (ad) must not count as an active bounty.  */
...
TEST_F (JobsTableTests, AdmissionCapCounts)
{
  ...
  CreateAd (Faction::RED, "alice", 1, 0, 10, 42).reset ();
  CreateAd (Faction::RED, "alice", 1, 0, 10, 42).reset ();
  CreateAd (Faction::RED, "bob", 1, 0, 10, 7).reset ();
...
TEST_F (JobsTableTests, ReservedCoins)
{
  ...
  CreateAd (Faction::RED, "poster", 2000, 8000, 10, 1).reset ();
  {
    auto j = CreateAd (Faction::RED, "poster", 500, 8000, 10, 1);
    j->SetWorker ("worker");
    j->SetStatus (Job::Status::ACCEPTED);
  }
...
{
  {
    auto j = CreateAd (Faction::RED, "poster", 2000, 500, 1234, 42);
    j->SetWorker ("worker");
    j->SetStatus (Job::Status::ACCEPTED);
    j->MutableProto ().mutable_ad ();
  ...
  EXPECT_EQ (e->GetType (), Job::Type::AD);
...
{
  {
    auto j = CreateAd (Faction::RED, "poster", 1, 0, 10, 1);
    tbl.WriteHistory (*j, JobOutcome::CANCELLED, 1, 300);
  }
  {
    auto j = CreateAd (Faction::RED, "poster", 2, 0, 10, 1);
    tbl.WriteHistory (*j, JobOutcome::VOID, 2, 100);
  }
  {
    auto j = CreateAd (Faction::RED, "poster", 3, 0, 10, 1);
...
TEST_F (JobHistoryTests, PruneBatched)
{
  ...
  for (int i = 0; i < 5; ++i)
    {
      auto j = CreateAd (Faction::RED, "poster", i + 1, 0, 10, 1);
