...
public:
  ...
  {
    INVALID = -1,
    /** Standing wanted-bounty pool, settled per qualifying kill of the target
        (the one type driven by the combat kill-hook, not a two-party escrow).  */
    WANTED = 0,
    /** Ad-slot rental on a building, settled at the calendar window's end
        (a scheduling/exclusivity mechanic, not a two-party escrow).  */
    AD = 1,
    /** Generic escrow deal: reward+collateral escrow, arbiter p-dial dispute
        settlement, the primitive that replaces the per-type verification jobs.  */
    DEAL = 2,
  };
...
private:
  ...
  /** Type-specific proto data (the designated worker, the payload, ...).  */
...
public:
   ...
   * when a linked entity (e.g. an ad-slot building) is destroyed.
   */
  Database::Result<JobResult> QueryForLinkedId (Database::IdT entity);

  /**
   * Queries for all jobs whose linked_name is the given account -- the wanted
   * bounty pools on that name.  Used by the kill attribution when one of the
   * account's characters dies.
   */
  Database::Result<JobResult> QueryForLinkedName (const std::string& name);

  /**
   * Returns whether ANY account is the target of an active wanted bounty (a
   * non-NULL linked_name exists): a single
   ...
   * linked to one entity, and the wanted bounty pools on one target
   * name.
   */
